...
Identical root-owned dependency files are hardlinked across retained releases before upload admission and after extraction. File bytes and permissions are preserved; writable files, symlinks, compiler caches, application caches and runtime configuration are excluded. This saves disk without deleting the previous release. Never modify installed dependency files in place.
